...
export const Track: Model<ITrackDoc> =
  mongoose.models.Track ?? mongoose.model<ITrackDoc>('Track', trackSchema);

/** Derive focus areas from subtitle (comma-separated) when not provided. */
function subtitleToFocusAreas(subtitle: string): string[] {
...
function getFocusAreas(
  doc: { id: string; subtitle: string; focusAreas?: string[] }
): string[] {
  if (doc.focusAreas?.length) return doc.focusAreas;
  const defaultTrack = DEFAULT_TRACKS.find((t) => t.id === doc.id);
  if (defaultTrack?.focusAreas?.length) return defaultTrack.focusAreas;
...
export async function getTracks(): Promise<TrackItem[]> {
  ...
  if (items.length === 0) return DEFAULT_TRACKS;
  return items.map((doc: { id: string; title: string; subtitle: string; image: string; domain: string; focusAreas?: string[] }) => ({
    id: doc.id,
    title: doc.title,
...
export function getPreferredTracks(tracks: TrackItem[], primaryFocus: string | null): TrackItem[] {
  const max = 4;
  if (!primaryFocus) return tracks.slice(0, max);
  const preferredDomains = FOCUS_TO_DOMAINS[primaryFocus] ?? [];
  if (preferredDomains.length === 0) return tracks.slice(0, max);
  ...
  if (isTechFocus) {
    ...
    if (systemDesign) return [...first3, systemDesign].slice(0, max);
