 ...
 */
export async function getAdminAnnouncementById(req, res) {
    if (!isAdminAuthorized(req)) {
        res.status(401).json({ success: false, error: 'Unauthorized' });
        return;
    }
 ...
 */
export async function postAdminAnnouncement(req, res) {
    if (!isAdminAuthorized(req)) {
        res.status(401).json({ success: false, error: 'Unauthorized' });
        return;
    }
 ...
 */
export async function putAdminAnnouncementById(req, res) {
    if (!isAdminAuthorized(req)) {
        res.status(401).json({ success: false, error: 'Unauthorized' });
        return;
    }
    try {
        ...
        const doc = await Announcement.findByIdAndUpdate(id, {
            $set: {
 ...
 */
export async function deleteAdminAnnouncementById(req, res) {
    if (!isAdminAuthorized(req)) {
        res.status(401).json({ success: false, error: 'Unauthorized' });
        return;
    }
 ...
 */
export async function patchAdminAnnouncementActivate(req, res) {
    if (!isAdminAuthorized(req)) {
        res.status(401).json({ success: false, error: 'Unauthorized' });
        return;
    }
 ...
 */
export async function getAdminAnnouncement(req, res) {
    if (!isAdminAuthorized(req)) {
        res.status(401).json({ success: false, error: 'Unauthorized' });
        return;
    }
    try {
        ...
        if (!doc)
            doc = await Announcement.findOne({ slug: 'current' }).lean();
        if (!doc)
            doc = await Announcement.findOne().sort({ updatedAt: -1 }).lean();
        if (!doc) {
            res.json({ success: true, data: null });
            return;
        }
 ...
 */
export async function putAdminAnnouncement(req, res) {
    if (!isAdminAuthorized(req)) {
        res.status(401).json({ success: false, error: 'Unauthorized' });
        return;
    }
    try {
        ...
        if (!doc)
            doc = await Announcement.findOne({ slug: 'current' });
        if (!doc) {
        ...
        if (typeof body.compulsory === 'boolean')
            doc.compulsory = body.compulsory;
        if (typeof body.title === 'string')
            doc.title = body.title;
        if (typeof body.message === 'string')
            doc.message = body.message;
        if (typeof body.openButtonLabel === 'string')
            doc.openButtonLabel = body.openButtonLabel;
        if (typeof body.cancelButtonLabel === 'string')
            doc.cancelButtonLabel = body.cancelButtonLabel;
        if (typeof body.url === 'string')
            doc.url = body.url;
        if (typeof body.version === 'number')
            doc.version = body.version;
        if (body.type !== undefined)
            doc.type = body.type || null;
        if (body.targetAppVersion !== undefined)
            doc.targetAppVersion = body.targetAppVersion || null;
        await doc.save();
        const d = doc.toObject();
        res.json({
            success: true,
