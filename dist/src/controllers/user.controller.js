...
import { getSessionsForUser, getSessionByIdForUser, addSessionForUser, clearSessionsForUser, removeSessionForUser, } from '../models/Session.js';
...
export async function createOrUpdateUser(req, res) {
    try {
        const userId = req.params.userId;
        const { displayName, avatarUri, photoURL, primaryFocus, email } = req.body;
        if (!userId) {
            res.status(400).json({ success: false, error: 'userId is required' });
            return;
        }
        let doc = await User.findOneAndUpdate({ firebaseUid: userId }, {
...
export async function getPreferredTracksHandler(req, res) {
    try {
        const userId = req.params.userId;
        if (!userId) {
            res.status(400).json({ success: false, error: 'userId is required' });
...
export async function addSession(req, res) {
    try {
        const userId = req.params.userId;
        const { domain, trackId, difficulty, focusTopic, durationSeconds, feedback } = req.body;
        if (!userId) {
            res.status(400).json({ success: false, error: 'userId is required' });
            return;
        }
